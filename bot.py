#!/usr/bin/env python3
"""
🚀 CloudX Hosting Bot - v4.2 (واجهة الأدمن المخصصة) – مع إصلاح JobQueue
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- تم إضافة JobQueue يدوياً لضمان عدم انهيار البوت في Railway.
...
# ─── تنظيف الجلسات المنتهية (تعمل كل ساعة) ──────────────────────────────────
async def cleanup_expired_sessions(ctx: ContextTypes.DEFAULT_TYPE):
    now = datetime.now()
    expired = []
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode=ParseMode.HTML,
        reply_markup=main_kb(uid)   # ✅ تمرير uid
    )
...
def main():
    ...
    # ✅ إنشاء JobQueue يدوياً لضمان عدم None في بيئة Railway
    job_queue = JobQueue()
    app = (
        Application.builder()
        .token(TOKEN)
        .job_queue(job_queue)            # ربط JobQueue صراحة
        .concurrent_updates(True)        # دعم التزامن
        .build()
    )

    # ✅ الآن job_queue موجود دائماً
    app.job_queue.run_repeating(cleanup_expired_sessions, interval=3600, first=60)
    ...
    log.info("🚀 CloudX Bot v4.2 يعمل مع JobQueue مضمون!")
